...
# 提醒开关控制
_enable_error_sound = True      # 错误提示音
_enable_error_email = True      # 错误邮件
_enable_success = True          # 成功提示音
...
def _on_prompt_error(exception=None):
    # 分别检查声音和邮件开关
    with _enable_lock:
        play_sound = _enable_error_sound
        send_mail = _enable_error_email

    if not play_sound and not send_mail:
        print("[ErrorSound] 🔇 所有错误提醒均已关闭，跳过")
        return
    ...
    if send_mail:
        if _email_config["enabled"]:
            ...
            print("[ErrorSound] ⚠️ 邮件未配置，但邮件提醒已开启，无法发送")
        ...
        try:
            ...
            def patched_task_done(self, item_id, history_result, status, process_item=None):
                ...
                if self.get_tasks_remaining() == 0:
                    ...
                    # 工作流结束后重置提醒状态为默认（全部开启）
                    with _enable_lock:
                        _enable_error_sound = True
                        _enable_error_email = True
                        _enable_success = True
                    print("[ErrorSound] 🔔 提醒状态已重置为默认（全部开启）")
...
class NotificationMute:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "any_input": ("*", {}),                 # 任意输入
                "enable_error_sound": ("BOOLEAN", {"default": True, "tooltip": "失败时播放提示音"}),
                "enable_error_email": ("BOOLEAN", {"default": True, "tooltip": "失败时发送邮件"}),
                "enable_success_sound": ("BOOLEAN", {"default": True, "tooltip": "全部成功时播放提示音"}),
            }
        }
    RETURN_TYPES = ("STRING",)
    FUNCTION = "execute"
    CATEGORY = "utils"
    DESCRIPTION = "分别控制失败声音、失败邮件、成功声音。工作流结束后自动恢复默认全部开启。"

    def execute(self, any_input, enable_error_sound, enable_error_email, enable_success_sound):
        global _enable_error_sound, _enable_error_email, _enable_success
        with _enable_lock:
            _enable_error_sound = enable_error_sound
            _enable_error_email = enable_error_email
            _enable_success = enable_success_sound
        status = (f"提醒设置: 错误声音{'开' if enable_error_sound else '关'} | "
