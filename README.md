...
成功提示音可以设置单个任务完成/所有任务完成触发  
需要更换提示音、加入邮箱提醒，进入notify.py填写数据  （成功默认关闭邮箱提醒）  
如果想在某个工作流里设置，拖入节点Notification Control，输出连接至展示文本（必须，不然节点不会执行），自行设置，最好还是在代码里设置，节点里的设置在任务失败后偶尔会恢复代码默认，目前搞不明白原因
  <br><br><br>
comfyui崩溃提醒  
